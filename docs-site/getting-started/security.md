...
!!! warning "The password gate is not a firewall"
    Either way, the built-in auth is a single-password gate — not a substitute for proper network controls. For real public exposure, put the server behind your own reverse proxy and keep it on loopback.
