...
What pAInapple Code adds is the interface: a per-session switcher instead of a startup flag, approval prompts rendered as cards in the chat, and a colored input border so you always know which session is armed and which is read-only. The list of modes comes from the [engine](engines.md) the session runs on — the Claude engine offers these six, and the Codex engine offers Codex's own sandbox tiers instead.
