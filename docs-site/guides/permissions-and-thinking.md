...
The colored button beside the input shows the current mode; click it to pick a level, roughly ordered from most restrictive to most permissive — roughly, because **Don't Ask** sits above **Ask** in the list while actually granting *less* (it auto-denies rather than asking you). The list comes from the [engine](engines.md) the session runs on — the Claude engine offers these six; the Codex engine offers its own sandbox tiers instead (Read-only / Workspace write / Full access). The input border takes on the mode's color so you always know which session is armed and which is read-only.
...
!!! note "Other engines"
    Sessions can also run on OpenAI Codex — see [AI engines](engines.md) for picking one per session (or pinning a server-wide default with `--default-provider`). Codex doesn't use these modes at all: its permission button maps to Codex **sandbox tiers** (Read-only / Workspace write / Full access) instead.
...
Your chosen mode is saved with the session and restored when you reopen it. On the default SDK engine a mode switch applies **immediately, even mid-turn** — the running engine changes gear in place, so you can flip to Accept Edits while approval cards are stacking up and the rest of the turn follows the new mode (the system log confirms "applied to the running engine"). Whenever the live switch isn't possible, it takes effect on your next message instead, when the server respawns the idle engine process with the new mode. The **Set as default** button in the popup makes the current mode the global default for new sessions (also editable in Settings).
