...
#   - A Python venv with the bridge's requirements
#   - A `painapple-code-start` launcher script at /usr/local/bin
#   - (optional) a /etc/profile.d hook that auto-starts the bridge on shell entry
...
# developer-tools layer, so the in-container PTY (which the bridge exposes
...
apt-get update
# Core runtime — what the bridge itself needs.
...
# Debian ships `fd` as `fdfind`; the bridge's api_files.py looks for `fd`.
...
#   2. Bundled `bridge-source/` next to the script — populated by
...
BUNDLED_SOURCE="$FEATURE_DIR/bridge-source"
...
# --- 5. Ownership ----------------------------------------------------------
# The bridge's state dir (PAINAPPLE_CODE_HOME) lives under the container
...
# bridge is already serving on $PORT.
cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
# Launch the pAInapple Code bridge in the background.
...
# The bridge's PTY spawner (routes/api_terminal.py) reads \$SHELL; whatever
...
# (where the bridge is already up) still gets the URL.
print_login_url() {
    local pw url
...
# --tls off: the bridge binds 0.0.0.0 inside the container, but the only
...
# Auto-start the pAInapple Code bridge on first interactive shell.
# Generated by the painapple-code Dev Container Feature.
...
# Auto-start the pAInapple Code bridge on first interactive fish session.
# Generated by the painapple-code Dev Container Feature.
...
if [ "$INSTALLHELPERS" = "true" ] && [ -x "$INSTALL_DIR/tools/install-helpers.sh" ]; then
    ...
    # would then auto-start the bridge during the image build, baking the
...
# bridge is already up on the first real container start. The
